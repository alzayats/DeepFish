...
def main(exp_dict, savedir):
    ...
    if not os.path.exists(fname_score_dict):
        ...
        habitats = set([l.split("/")[0] for l in val_set.clf.img_names])
        ...
        for habitat in habitats:
            ...
            val_set = datasets.get_dataset(dataset_name=exp_dict["dataset"], split="val",
                                ...
                                tasks=exp_dict["tasks"])
            val_loader = DataLoader(val_set, shuffle=False, batch_size=exp_dict["batch_size"])
            # print(val_set.clf.img_names)
            habitat_name = cg.Habitats_dict[habitat.split("/")[-1]]
            # print(val_set.clf.labels)
            # score_dict["%s_%s" % (habitat_name, model)] = func(val_set.clf.labels)
            score_dict["%s_n_samples" % (habitat_name)] = len(val_set)
            val_dict = trainers.val_epoch(model, val_loader)
            score_dict["%s" % (habitat_name)] = val_dict[val_key]
...
def baselines(dataset="Fish", model="always-0"):
    ...
    score_dict["%s" % (habitat_name)] = func(val_set.clf.labels)
    score_dict["%s_n_samples" % (habitat_name)] = len(val_set)
...
if __name__ == "__main__":
    ...
    results = []
    for model in ["always-0", "always-1"]:
    ...
    # print(df)
    # df.to_csv(savedir_base + "/csv_results/clf.csv")
    ...
    df_filter = df_filter[["habitat", "always-0", "always-1", "ResNet", "DeepFish"]]
