...
def main(exp_dict, savedir):
    fname_score_dict = savedir + "/test_score_dict.json"
    if not os.path.exists(fname_score_dict):
        ...
        habitats = set([l.split("/")[1].split("_")[0] for l in val_set.reg.img_names])
        ...
        for habitat in habitats:
            ...
            val_set = datasets.get_dataset(dataset_name=exp_dict["dataset"], split="val",
                                ...
                                tasks=exp_dict["tasks"])
            val_loader = DataLoader(val_set, shuffle=False, batch_size=exp_dict["batch_size"])
            # print(val_set.reg.img_names)
            habitat_name = cg.Habitats_dict[habitat]
            # print(val_set.reg.labels)
            # score_dict["%s_%s" % (habitat_name, model)] = func(val_set.reg.labels)
            score_dict["%s_n_samples" % (habitat_name)] = len(val_set)
            val_dict = trainers.val_epoch(model, val_loader)
            score_dict["%s" % (habitat_name)] = val_dict[val_key]
...
def baselines(dataset="Fish", model="always-0"):
    score_dict = {}
    dataset_dir = cg.DATADIR_BASE_DICT[dataset]["issam"]
    
    train_set = datasets.get_dataset(dataset_name=dataset, split="train",
                                transformer_name="rgb_normalize",
                                dataset_dir=dataset_dir,
                                tasks=tasks)
    if model == "always-mean":
        func = lambda x: np.abs(x-train_set.reg.counts.mean()).mean()
    val_set = datasets.get_dataset(dataset_name=dataset, split="val",
                                transformer_name="rgb_normalize",
                                dataset_dir=dataset_dir,
                                tasks=tasks)
    habitats = set([l.split("/")[1].split("_")[0] for l in val_set.reg.img_names])
    ...
    score_dict["%s" % (habitat_name)] = func(val_set.reg.counts)
    score_dict["%s_n_samples" % (habitat_name)] = len(val_set)
...
if __name__ == "__main__":
    ...
    results = []
    for model in ["always-mean"]:
        results += [baselines(dataset="Fish", model=model)]
    ...
    df_filter = df_filter[["habitat", "always-mean", "ResNet", "DeepFish"]]
